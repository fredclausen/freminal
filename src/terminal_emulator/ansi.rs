...
impl CsiParser {
    ...
    fn from_snapshot(snapshot: SnapshotItem) -> Result<Self, LoadCsiParserSnapshotError> {
        use LoadCsiParserSnapshotError::{
            InvalidState, ItemNotU8, ItemNotVec, MissingItem, RootItemNotMap, StateNoFinsihedValue,
            StateNoType, StateNotMap, StateTypeNotString,
        };
...
impl FreminalAnsiParser {
    ...
    pub fn from_snapshot(snapshot: SnapshotItem) -> Result<Self, LoadSnapshotError> {
        use LoadSnapshotErrorKind::{MissingElem, UnknownElem, WrongType};
...
mod test {
    ...
    fn test_ansi_parser_snapshot() {
        for inner in [
            ...
            let parser = FreminalAnsiParser { inner };
            let loaded = FreminalAnsiParser::from_snapshot(parser.snapshot())
