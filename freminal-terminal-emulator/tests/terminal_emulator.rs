...
fn test_format_tracker_scrollback_split_on_boundary() {
    ...
    let res = split_format_data_for_scrollback(tags.clone(), 10, usize::MAX, true);
...
fn test_format_tracker_scrollback_split_segment() {
    ...
    let res = split_format_data_for_scrollback(tags, 9, usize::MAX, true);
    assert_eq!(
        res.scrollback,
        &[
            FormatTag {
...
fn test_format_tracker_scrollback_no_split() {
    let tags = get_tags();
    // Case 1: no split
    let res = split_format_data_for_scrollback(tags.clone(), 0, usize::MAX, true);
